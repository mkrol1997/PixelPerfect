...
def get_gdrive_folder_id(credentials: Credentials) -> str:
    ...
    while True:
        response = drive.files().list(
            q=f"mimeType='application/vnd.google-apps.folder' and name = '{settings.GOOGLE_DRIVE_FOLDER_NAME}'",
            spaces='drive',
...
def uploadImage(credentials, src_image_name, save_image_name, mime_type, folder_id):

    drive = googleapiclient.discovery.build(
        'drive', 'v3', credentials=credentials)

    file_metadata = {'name': 'save_image_name', 'parents': [folder_id]}
    media = MediaFileUpload(src_image_name, mimetype=mime_type)
