...
def test_split_fn(model, L, refield=32, min_size=256, sf=1, modulo=1):
    ...
    if h * w <= min_size**2:
        L = torch.nn.ReplicationPad2d(
            (0, int(np.ceil(w / modulo) * modulo - w), 0, int(np.ceil(h / modulo) * modulo - h))
        )(L)
        E = model(L)
        E = E[..., : h * sf, : w * sf]
    else:
        ...
        if h * w <= 4 * (min_size**2):
            ...
            Es = [
                test_split_fn(model, Ls[i], refield=refield, min_size=min_size, sf=sf, modulo=modulo) for i in range(4)
            ]
...
def test_split_x8(model, L, refield=32, min_size=256, sf=1, modulo=1):
    E_list = [
        test_split_fn(
            model, util.augment_img_tensor4(L, mode=i), refield=refield, min_size=min_size, sf=sf, modulo=modulo
        )
        for i in range(8)
    ]
...
def describe_params(model):
    ...
    for name, param in model.state_dict().items():
        if not "num_batches_tracked" in name:
            ...
            msg += (
                " | {:>6.3f} | {:>6.3f} | {:>6.3f} | {:>6.3f} || {:s}".format(v.mean(), v.min(), v.max(), v.std(), name)
                + "\n"
            )
    return msg
